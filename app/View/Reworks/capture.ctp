...
$this->Html->script('controllers/Reworks/capture', array('block' => 'scriptBottom'));
$this->Html->css('controllers/Reworks/capture', array('block' => 'stylesTop'));
$units = Units::symbolHtml(Units::UNITS);
$urlCreate = $this->Html->url(array('action' => 'create'));
$urlDelete = $this->Html->url(array('action' => 'delete'));
$this->TioCachas->templateSwig('templateRow', 'rework-user');
?>
<div>
...
<div class="table-responsive" id="records">
    <table class="table table-bordered table-condensed table-striped">
        ...
        <tbody>
            <?php foreach ($reworks as $r): ?>
                <tr data-id="<?php echo $r['rId']; ?>">
                    <td><?php echo $r['rValue'] . ' ' . $units; ?></td>
                    <td><?php echo $r['rComment']; ?></td>
                    <td>
