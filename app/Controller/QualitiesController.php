...
class QualitiesController extends AppController {
    ...
    public function capture($operationId) {
        ...
        if (isset($operation['Operation']) === true) {
            $qualities = $this->Quality->getByOperationId($operationId, array(Quality::STATUS_ENABLED));
    ...
    public function create() {
        ...
        if (isset($params['o']) && isset($params['v']) && isset($params['c'])) {
            ...
            $comment = $params['c'];
            $newRecord = $this->Quality->insert($operationId, $value, $comment);
        }
