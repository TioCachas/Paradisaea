...
//        }
    ...
    public function get()
    {
        $userSesion = $this->Auth->user();
        $userId = $userSesion['id'];
        $operations = $this->Operation->getCurrentsByUserId($userId, array(Operation::STATUS_ENABLED));
        $this->set(array('operations' => $operations, '_serialize' => 'operations'));
        $this->viewClass = 'Json';
    }
    ...
    public function create()
    {
        ...
        $line = $this->Line->findById($lineId);
        $hour = $this->Hour->findById($hourId);
        $newOperation = $this->Operation->insert($userId, $lineId, $hourId, $scrap, $rework, $changeover, $technicalLosses, $organizationalLosses, $qualityLosses, $performanceLosses, $workDate);
