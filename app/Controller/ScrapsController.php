...
class ScrapsController extends CrudController
{
    /// --------------I N I C I O   C R U D-------------------------------------
    public $_model = 'Scrap';
    ...
    public function admin($operationId)
    {
        ...
        if (isset($operation['Operation']) === true)
        {
            ...
            $workstationsByLine = $this->Workstation->getEnabledByLineAndType2($lineId, Workstation::TYPE_SCRAP);
            if (count($workstationsByLine) > 0)
            {
                ...
                if (count($defectsByLine) > 0)
                {
                    ...
                    $appBosch->type = Workstation::TYPE_SCRAP;
                    /**
